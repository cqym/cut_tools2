...
Ext.apply(_config, getConfig());
    ...
    var grid = new Ext.grid.GridPanel({
		...
		tbar:[{
				...
				listeners : {
					'click' : function() {
						Ext.ffc.select_quotations_delivery('3','6',function(arr,select_quotations_win){
									...
									success: function(response){
											...
											select_quotations_win.close();//关闭报价选择窗口
												var conEditWin = new Ext.ffc.QuotationOutStockEditWindow({			
																		title :  '新增出库单',
																		outStockInfor:{quotationId:quotationId,userName:LoginInfor.user.trueName,customerCode:customerCode,customerName:customerName,status:0,outStockType:5,quotationCode:quotationCode,memo:memo,outStockDetails:detail},
																		listeners :{
																			close : function(p){
				...
				listeners : {
					'click' : function(obj) {
						...
						window.open(PATH + "/outStock/outStockOutExcelAction.do?ffc=expertList2Excel&outStockType=5&" + para.join("&"));
					}
				}
        	},*/{
        		text:'导出列表',
