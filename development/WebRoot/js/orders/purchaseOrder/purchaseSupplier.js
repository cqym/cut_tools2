...
Ext.ffc.PurchaseOrder.cOSupplierGrid = Ext.extend(Ext.grid.GridPanel,{
		store:null,
		constructor : function(_cfg){
			...
			Ext.apply(this, _cfg);
			var hpurl = PATH + "/reserveOrder/suppliersInforList.do";
			if(this.contractId){
				hpurl = PATH + "/contractOrder/supplierList.do?contractId=" + this.contractId + "&leaf="+this.leaf + "&brand=" + this.brand+"&toolsTypeName="+this.toolsTypeName;
			}else if(this.quotationId){
			    hpurl = PATH + "/scheduleOrder/supplierList.do?qId=" + this.quotationId + "&leaf="+this.leaf+"&brand=" + this.brand+"&toolsTypeName="+this.toolsTypeName;
			}else if(this.planIds){
			    hpurl = PATH + "/planOrder/suppliersInforList.do?planId='" + this.planIds.join(',') + "'";
			}
			var proxy = new Ext.data.HttpProxy({url: hpurl});
			...
			Ext.ffc.PurchaseOrder.cOSupplierGrid.superclass.constructor.call(this, {
				height:520,
				store: this.store,
				columns: [
...
Ext.ffc.PurchaseOrder.cOSupplierSelectForm = Ext.extend(Ext.FormPanel, {
	constructor : function(_cfg) {
		if(_cfg == null) {
			_cfg = {};
		}
		...
		Ext.ffc.PurchaseOrder.cOSupplierSelectForm.superclass.constructor.call(this, {
	        ...
	        items:[
	           ...
	           items:[
				  ...
				  {items: [{xtype:'textfield',fieldLabel: '品牌',name: 'brand',anchor:'90%',value:brandV}]}
	           ]//items
	          }
	        ],
			keys : {
...
Ext.ffc.PurchaseOrder.cOSupplierWin = Ext.extend(Ext.Window,{
		supplier_grid:null,
		contract_code:null,
		customerCode:null,
		customerName:null,
		...
		constructor : function(_cfg){
			...
			Ext.apply(this, _cfg);
			this.flag = false;
			this.selectForm2 = new Ext.ffc.PurchaseOrder.cOSupplierSelectForm({brand:this.brand});
			this.supplier_grid = new Ext.ffc.PurchaseOrder.cOSupplierGrid({
				contractId:this.contractId,
				brand:this.brand,
			  quotationId:this.quotationId,
			  leaf:this.leaf,
			  planIds:this.planIds,
			  toolsTypeName : this.toolsTypeName
			});
			this.supplier_grid.on('rowdblclick',function(grid,rowIndex,e)
			{
				var s = grid.getStore();
				var record = s.getAt(rowIndex);
			      this.callBackMethod(record.get("id"));
			},this);
			Ext.ffc.PurchaseOrder.cOSupplierWin.superclass.constructor.call(this, {
