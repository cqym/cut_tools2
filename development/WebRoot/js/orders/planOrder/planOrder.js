...
Ext.apply(_config, getConfig());
	...
	cut_tools.plan_order.index_grid = new Ext.grid.GridPanel({
		...
		columns: [
			...
			{header: "合同编号",dataIndex: "contractCode",width: 180,hidden:true},
			{header: "订单编号",dataIndex: "orderCode",width: 180,sortable:true,renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
					if(record.data.contractCode!=""&&record.data.contractCode!=null)
					{
						return '<span style="color:red"  title='+record.data.contractCode+'>' +value + "</span>";
					}
					else
					{
			 ...
			 tbar : [{
				...
				listeners: {
					'click' : function(){
							...
							Ext.Ajax.request({
								...
								params: { ids: ids ,'method':'submitPlanOrder'},
								success: function(response){
				...
				listeners: {
					'click' : function(){	
						...
						window.open(PATH + '/contractOrder/orderListExcel.do?orderType=4&searchForm='+Ext.util.JSON.encode(new Ext.data.Record(selectForm2.getForm().getValues())));
					     */
...
Ext.ffc.showSlaveFillWindow = function (data){
	...
	Ext.ffc.ResizeManager.addResizeObject(cut_tools.plan_order.index_win);
})
