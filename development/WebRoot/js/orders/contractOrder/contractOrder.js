...
Ext.onReady(function(){
	...
	var proxy = new Ext.data.HttpProxy({url: PATH + "/contractOrder/listAction.do"});
...
Ext.apply(_config, getConfig());
	...
	store.baseParams.searchStr = Ext.encode({'year':new Date().format('Y')});
			 ...
			 tbar : [
			{
				...
				listeners: {
					'click' : function(){
						var win = new Ext.ffc.PurchaseOrder.cOContractWin({
							  leaf:1,
						    callBackMethod:function(contractInfors){
							  var contractId = contractInfors.contractId;
							  var  customerCode = contractInfors.customerCode;
							  var  contractCode = contractInfors.contractCode;
							  var  brand = contractInfors.productBrand;
							  var  toolsTypeName = contractInfors.toolsTypeName;
						      if(!brand){
						          brand = '';	
						      }
							  if(!toolsTypeName){
																	...
																	var win = new Ext.ffc.PurchaseOrder.cOSupplierWin({
																		...
																		callBackMethod:function(suppid){
																			...
																			loadOrderWindowByConsult({
																					    ...
																					    loadDataParam:{contractId:contractId,supplierId:suppid,brand:brand,leaf:1,loadActionMethod:'consultContractInfor',toolsTypeName:toolsTypeName}
																			});
																			...
																			var win = new Ext.ffc.PurchaseOrder.cOSupplierWin({
																				...
																				callBackMethod:function(suppid){
																			     ...
																			     loadOrderWindowByConsult({
																					    ...
																					    listGrid:cut_tools.contract_order.index_grid,
																					    loadDataParam:{
																					    	contractId:contractId,supplierId:suppid,brand:brand,leaf:1,loadActionMethod:'consultContractInfor',toolsTypeName:toolsTypeName
																					    }
																				   });
				...
				listeners: {
					'click' : function(){
							...
							Ext.Ajax.request({
								url: PATH + '/contractOrder/CheckOrderAmountAction.do',
								params: { ids: ids },
								success: function(response){
									if(response.responseText != ''){
										...
										Ext.Ajax.request({
											...
											params: {'method':'submitContractOrder', ids: ids },
											success: function(response){
												if(response.responseText != ''){
													Ext.Msg.alert("消息", response.responseText);
												}else{
				...
				listeners: {
					'click' : function(){	
							...
							Ext.apply(record.data,{'quotationCode':"",'userName':''});
...
Ext.ffc.lableStyle_ = "font-size:9pt;text-align:right;width:100px";
Ext.ffc.lableStyle2_ = "font-size:9pt;text-align:right;width:70px";
	...
	var  selectForm2 = new Ext.FormPanel({
		labelAlign:'right',buttonAlign:'right', border : false,
	        ...
	        items : [
						//1
						{xtype:'label',text:'订单编号:',x:-40,y:5,style:Ext.ffc.lableStyle_},
						{xtype:'textfield', name: 'orderCode',x:60,y:3, width:170},

						{xtype:'label',text: '状态:',x:-40,y:35,style:Ext.ffc.lableStyle_},
						new Ext.ftl.StockOrderComboBox({x:60,y:33, width:170}),
						//2
						{xtype:'label',text: '合同编号:',x:210,y:5,style:Ext.ffc.lableStyle_},
						{xtype:'textfield',name: 'contractCode',x:310,y:3, width:170},

						{xtype:'label',text: '编制日期:',x:210,y:35,style:Ext.ffc.lableStyle_},
						{xtype:'datefield',name: 'startTime', readOnly : true, format:'Y-m-d',emptyText:'',x:310,y:33, width:170},
						//3
						{xtype:'label',text: '供应商:',x:470,y:5,style:Ext.ffc.lableStyle2_},
						{xtype:'textfield',name: 'supplierName',x:540,y:3, width:170},
					
						{xtype:'label',text: '至:',x:470,y:35,style:Ext.ffc.lableStyle2_},
						{xtype:'datefield',name: 'endTime', readOnly : true, format:'Y-m-d',emptyText:'',x:540,y:33, width:170},
					    //4
						{xtype:'label',text: '客户:',x:700,y:5,style:Ext.ffc.lableStyle_},
						{xtype:'textfield', name: 'customerName',x:800,y:3, width:170},

						{xtype:'label',text: '合同我方负责人:',x:700,y:35,style:Ext.ffc.lableStyle_},
						{xtype:'textfield', name: 'ownContactPerson',x:800,y:33, width:170},
						
						//5
						{xtype:'label',text: '供应商我方负责人:',x:970,y:5,style:Ext.ffc.lableStyle_},
						{xtype:'textfield', name: 'supplierOwnContactPerson',x:1070,y:3, width:60},

						{xtype:'label',text: '年度:',x:1130,y:5,style:this.lableStyle_},
						 new Ext.ffc.YearComboBox({x:1160,y:3, width:80}),

						{xtype:'label',text: '备注:',x:970,y:35,style:Ext.ffc.lableStyle_},
						{xtype:'textfield', name: 'memo',x:1070,y:33, width:170},

						{xtype:'hidden',fieldLabel: '订单类型',name: 'orderType',value:1}
					],
			keys : {
	...
	Ext.ffc.ResizeManager.addResizeObject(cut_tools.contract_order.index_win);
})
