...
function updateFlag(obj){
   ...
   Ext.Ajax.request({
		url: PATH + '/selfOrder/UpdateFlagAction.do',
		params: { id: obj.value,value:v },
...
Ext.apply(_config, getConfig());
	...
	var Bill = Ext.data.Record.create([
									   ...
									   {name: 'editedPlan',mapping:'editedPlan',type:'float'}
	]);
	...
	cut_tools.self_order.index_grid = new Ext.grid.GridPanel({
		...
		columns: [
			 new Ext.grid.RowNumberer(),
				...
				{header: "已做计划",dataIndex: 'editedPlan', width: 70,sortable:true,renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
						if(value * 1 == 1){
							return "<span style='color:#088A85;font-weight:bold;'>部分</span>";
						}else if(value * 1 == 2){
							return "<span style='color:green;font-weight:bold;'>全部</span>";
						}else{
							return "<span style='color:#A1A09D;font-weight:bold;'>未做</span>";
						}
					}
				},	
			 ...
			 tbar : [{
																	...
																	var win = new Ext.ffc.PurchaseOrder.cOSupplierWin({
																		...
																		callBackMethod:function(suppid){
																			...
																			loadOrderWindowByConsult({
																					    ...
																					    loadDataParam:{contractId:contractId,supplierId:suppid,brand:brand,leaf:0,loadActionMethod:'consultContractInfor',toolsTypeName:toolsTypeName}
																			});																			     
																		}
																	});
																	win.show();  
																			...
																			var win = new Ext.ffc.PurchaseOrder.cOSupplierWin({
																				...
																				callBackMethod:function(suppid){
																			     ...
																			     loadOrderWindowByConsult({
																					    ...
																					    listGrid:cut_tools.self_order.index_grid,
																					    loadDataParam:{
																					    	contractId:contractId,supplierId:suppid,brand:brand,leaf:0,loadActionMethod:'consultContractInfor',toolsTypeName:toolsTypeName  
																					    }
																				   });
				...
				listeners: {
					'click' : function(){
						...
						if(stu != 0 && stu != 3 && stu != 2)
						{
							Ext.Msg.show({
							   title: '系统提示',
							   msg: '只允许【编制】，【退回】，【审核通过】状态可以修改，所选订单不能被修改！',
							   width: 300,
							   buttons: Ext.MessageBox.OK,
							   icon: Ext.MessageBox.INFO
							});
				...
				listeners: {
					'click' : function(){
							...
							Ext.Ajax.request({
										...
										params: { ids: ids ,'method':'submitSelfOrder'},
											success: function(response){
...
Ext.ffc.showEditResservePlanWindow = function(process_orderId,ReservePlanListStore){
	...
	cut_tools.self_order.index_win = new Ext.Panel({
		...
		Ext.ffc.ResizeManager.addResizeObject(cut_tools.self_order.index_win);
	//监听生成计划事件
	EventMger.on("createdPlanEvent",function(){
		var store = cut_tools.self_order.index_grid.store;
		var searchStr = Ext.encode(selectForm2.getForm().getValues());
		store.baseParams.searchStr = searchStr;
		store.reload();
