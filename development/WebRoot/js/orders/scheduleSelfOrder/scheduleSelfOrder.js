...
Ext.apply(_config, getConfig());
	var store = new Ext.data.JsonStore({
				...
				fields : ['id',"orderCode","quotationCode","currencyId","currencyName","orderDate","orderType","status","supplierName","supplierId",
					...
					"overallRebate","finalMoney",'fileCount',"contractCode","editedPlan"]
			});
																	...
																	var win = new Ext.ffc.PurchaseOrder.cOSupplierWin({
																		...
																		callBackMethod:function(suppid){
																			...
																			loadOrderWindowByConsult({
																					    ...
																					    loadDataParam:{quotationId:quotationId,supplierId:suppid,brand:brand,leaf:0,loadActionMethod:'consultScheduleQuo',toolsTypeName:toolsTypeName}
																			});
																		}
																	});
																	win.show();  
				...
				listeners: {
					'click' : function(){
							...
							Ext.Ajax.request({
										...
										params: { ids: ids ,'method':'submitScheduleSelfOrder'},
											success: function(response){
...
Ext.ffc.showEditResservePlanWindow = function(process_orderId,ReservePlanListStore){
	...
	cut_tools.qs_order.index_win = new Ext.Panel({
		...
		Ext.ffc.ResizeManager.addResizeObject(cut_tools.qs_order.index_win);
})