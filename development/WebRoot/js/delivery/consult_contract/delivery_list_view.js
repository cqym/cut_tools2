...
var  selectForm3 = new Ext.FormPanel({
                        ...
                        items:[
                            ...
                            items:[
								...
								{items: [new Ext.ffc.DeliveryStatusComboBox({anchor:'90%'})]},  
...
Ext.apply(_config, getConfig());
    ...
    var grid = new Ext.grid.GridPanel({
		...
		tbar:[{
				...
				listeners : {
					'click' : function() {
						Ext.ffc.select_contracts(function(arr,select_quotations_win,customer){
							...
							checkCustomerAccountForDelivery({
								...
								if(fileCount * 1 == 0 && (customer != null && customer.contractRunCondi != -2)){
				...
				listeners : {
					'click' : function() {
							...
							dData["DeliveryEditCenterGridStore"] = Ext.ffc.getDeliveryEditCenterGridStore(PATH + '/delivery/deliveryViewAction.do?ffc=deliveryViewById&id=' + dData.id);
						    var conEditWin = new Ext.ffc.DeliveryEditWindow({deliveryInfor:dData,
								listeners :{
									close : function(p){
									    deliveryListStore.reload();
									}
				...
				listeners : {
					'click' : function() {
						...
						for(var i = 0 ;i < arr.length;i++){
						     ids.push(arr[i].id);
							 if(arr[i].get("status") == 4){
								Ext.Msg.alert("消息", "所选择交货单不允许作废!");
							    return ;
							 }
						}
	...
	//对客户欠款金额进行校验
function checkCustomerAccountForDelivery(config){
	Ext.Ajax.request({
		method: "post",
