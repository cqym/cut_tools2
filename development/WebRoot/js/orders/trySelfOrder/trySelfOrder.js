...
Ext.apply(_config, getConfig());
	var store = new Ext.data.JsonStore({
				...
				fields : ['id',"orderCode","quotationCode","currencyId","currencyName","orderDate","orderType","status","supplierName","supplierId",
					...
					"overallRebate","finalMoney",'fileCount','editedPlan']
			});
store.baseParams.year = new Date().format('Y');
	...
	cut_tools.ts_order.index_grid = new Ext.grid.GridPanel({
		...
		//{header: "供应商id",dataIndex: 'supplierId',width: 60,sortable:true,hidden:true},
			...
			{header: "编制时间",dataIndex: 'editDateCopy',width: 100,sortable:true},	
													...
													Ext.Ajax.request({
														...
														success: function(response){
																	...
																	var win = new Ext.ffc.PurchaseOrder.cOSupplierWin({
																		...
																		callBackMethod:function(suppid){
																			...
																			loadOrderWindowByConsult({
																					    title:'预定采购订单',
																					    listGrid:cut_tools.ts_order.index_grid,
																					    loadDataParam:{quotationId:quotationId,supplierId:suppid,brand:brand,leaf:0,loadActionMethod:'consultTryQuo',toolsTypeName:toolsTypeName}
																			});
																		}
																	});
																	win.show();  
																...
																var conEditWin = new Ext.ffc.ContractOutStockEditWindow({
																	outStockInfor:{quotationId:quotationId,outStockType:6,status:0,quotationCode:quotationCode,outStockDetails:detail},
				...
				listeners: {
					'click' : function(){
							...
							Ext.Ajax.request({
										...
										params: { ids: ids ,'method':'submitTrySelfOrder'},
											success: function(response){
	...
	var  selectForm2 = new Ext.FormPanel({
	        ...
	        items:[
	           ...
	           items:[
					...
					{items: [new Ext.ftl.StockOrderComboBox({anchor:'90%'})]},
...
//					{items: [{xtype:'textfield',fieldLabel: '合同我方负责人',name: 'ownContactPerson',anchor:'90%'}]},
	...
	cut_tools.ts_order.index_win = new Ext.Panel({
		...
		Ext.ffc.ResizeManager.addResizeObject(cut_tools.ts_order.index_win);
})
